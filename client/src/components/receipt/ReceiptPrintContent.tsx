      ...
      <div className="text-left space-y-0.5 text-[11px]">
        ...
        <div className="flex justify-between">
          ...
          <span className="font-bold">
            {order.serverStaffName ? `${order.serverStaffName} (${order.serverStaffId})` : (order.serverStaffId || 'Marco Rossi (W-101)')}
          </span>
        </div>
      </div>
            ...
            {item.selectedModifiers && item.selectedModifiers.length > 0 && (
              <div className="text-[10px] text-gray-500 pl-2">
                {item.selectedModifiers.map((m, mi) => (
                  <span key={mi}>
                    +{m.optionName} ({currencySymbol}{m.price.toFixed(2)}){' '}
                  </span>
                ))}
              </div>
            )}
      ...
      <div className="text-left space-y-1 text-[11px]">
        ...
        {tenderedAmount !== undefined && (
          <div className="flex justify-between text-gray-700">
            ...
            <span>{currencySymbol}{tenderedAmount.toFixed(2)}</span>
          </div>
        )}
